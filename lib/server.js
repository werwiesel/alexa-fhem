...
if( 0 ) {
      ...
      this.setreading( 'personId', personId );
      ...
      for( let fhem of this.connections ) {
        ...
        if( fhem.fhemIntents[intent_name] !== undefined ) {
          ...
          if( name.match(/^(set|get|attr)\s/) ) {
            ...
            fhem.execute( 'setreading '+ fhem.alexa_device.Name +' fhemIntent '+ intent_name + applicationId + ';setreading '+ fhem.alexa_device.Name +' personId '+ personId + ';setreading '+ fhem.alexa_device.Name +' echoId '+ echoId + ';setreading '+ fhem.alexa_device.Name +' echoRoom '+ echoRoom +';'+ name, function(result) {
...
//console.log(name);
            ...
            if( applicationId !== '' ) applicationId = ' :' +applicationId;
            fhem.execute( 'setreading '+ fhem.alexa_device.Name +' personId '+ personId + ';setreading '+ fhem.alexa_device.Name +' echoId '+ echoId + ';setreading '+ fhem.alexa_device.Name +' echoRoom '+ echoRoom + ';' + 'setreading '+ fhem.alexa_device.Name +' fhemIntent '+ name + applicationId );
          }
        }
      }
      if( match ) {
...
function propertiesFromDevice(device) {
  ...
  if( mapping = device.mappings.FanSpeed ) {
    var current = device.fhem.reading2homekit(mapping, device.fhem.cached(mapping.informId));
    if (current !== undefined) {
      properties.push({
...
if( 0 )
  ...
  if(device.mappings.FanSpeed ) {
    ...
    d.capabilities.push( {
          ...
          "configuration": {
            "supportedRange": {
              "minimumValue": (device.mappings.FanSpeed.minValue !== undefined ? device.mappings.FanSpeed.minValue : 0),
              "maximumValue": (device.mappings.FanSpeed.maxValue !== undefined ? device.mappings.FanSpeed.maxValue : 3),
              "precision": (device.mappings.FanSpeed.minStep !== undefined ? device.mappings.FanSpeed.minStep : 1)
            },
            "presets": [
              {
                "rangeValue": (device.mappings.FanSpeed.maxValue !== undefined ? device.mappings.FanSpeed.maxValue : 3),
                "presetResources": {
                  "friendlyNames": [
                    {
                      "@type": "asset",
  ...
  if( device.isOfType('blind') && device.mappings.TargetPosition ) {
    d.capabilities.push( {
	                   ...
	                   "capabilityResources": {
                           ...
                           "configuration": {
                             "supportedRange": {
                               "minimumValue": (device.mappings.TargetPosition.minValue !== undefined ? device.mappings.TargetPosition.minValue : 0),
                               "maximumValue": (device.mappings.TargetPosition.maxValue !== undefined ? device.mappings.TargetPosition.maxValue : 100),
                               "precision": (device.mappings.TargetPosition.minStep !== undefined ? device.mappings.TargetPosition.minStep : 1)
                             },
                             ...
                             "actionMappings": [
                               {
                                 ...
                                 "directive": {
                                   "name": "SetRangeValue",
                                   "payload": {
                                     "rangeValue": (device.mappings.TargetPosition.minValue !== undefined ? device.mappings.TargetPosition.minValue : 0)
                                   }
                                 }
                               },
                               {
                                 ...
                                 "directive": {
                                   "name": "SetRangeValue",
                                   "payload": {
                                     "rangeValue": (device.mappings.TargetPosition.maxValue !== undefined ? device.mappings.TargetPosition.maxValue : 100)
                                   }
                                 }
                               },
                               {
                             ...
                             "stateMappings": [
                               {
                                 ...
                                 "value": (device.mappings.TargetPosition.minValue !== undefined ? device.mappings.TargetPosition.minValue : 0)
                               },
                               {
                                 "@type": "StatesToRange",
...
var handleRangeController = function(event) {
  ...
  if( device.mappings.TargetPosition )
    mapping = device.mappings.TargetPosition;
  else if( device.mappings.FanSpeed )
    mapping = device.mappings.FanSpeed;
  else
    return createError(ERROR3_INVALID_DIRECTIVE, undefined, event);
  var current = parseInt( device.fhem.cached(mapping.informId) );
